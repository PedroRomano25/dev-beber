...
import { CardDrinkOptionsDefault, CardDrinkOptionsFilter, CardDrinkOptionsSemResultado } from "../../Data/allDatas";
import { axiosAppServer } from "../../Service/axiosAppServer";
import { ConvertToDrinks, ConvertToOptionsAlcoholic, ConvertToOptionsCategory, ConvertToOptionsGlass, ConvertToOptionsIngredients } from "../../Service/convertToOptions";
...
const PaginaInicial = () => {
  ...
  const [DrinksShow,setDrinksShow] = useState<CardDrinkOptions[]>([...CardDrinkOptionsDefault])
  const [Filters,setFilters] = useState<CardDrinkOptions>(CardDrinkOptionsFilter)
  const contexto = useApiCocktailContext();

  const getDatabyLetterSelected = async (letra:string) => {
    let dados = await axiosAppServer.get('/api/json/v1/1/search.php?f='+letra)
        .then((i: any) => {        
...
const getDatabySearch = async (pesquisa:string) => {
  let dados = await axiosAppServer.get('/api/json/v1/1/search.php?s='+pesquisa)
      .then((i: any) => {
        if (i.data.drinks != null){ setDrinksShow(i.data.drinks);}
        if (i.data.drinks === null){ setDrinksShow(CardDrinkOptionsSemResultado);}

      })
      .catch((i) => console.log("Falha ao coletar dados" + i)) 
  return dados
}
console.log("LetraSelecionada",LetraSelecionada)
  ...
  const dadoMemo = useMemo(() => {  
    return (DrinksShow
      .filter((i) => Filters.strGlass === undefined || Filters.strGlass === null ? i === i : i.strGlass === Filters.strGlass)
      .filter((i) => Filters.strCategory === undefined || Filters.strCategory === null ? i === i : i.strCategory === Filters.strCategory)
      .filter((i) => Filters.strAlcoholic === undefined || Filters.strAlcoholic === null ? i === i : i.strAlcoholic === Filters.strAlcoholic)
      .filter((i) => Filters.strIngredients === undefined || Filters.strIngredients === null || Object.values(i).includes(Filters.strIngredients) ? i === i : null)      
      )
  },[DrinksShow,Filters])
    ...
    <>
      <HorizontalAlign>
        ...
        <Autocomplete
          ...
          sx={{ width: "15vw", backgroundColor: "var(--nav-bg)" }}
          renderInput={(params) => <TextField {...params} label="Categoria de Alcool" />}
        />
      </HorizontalAlign>
